...
public interface StatsRepository extends JpaRepository<Hit, Long> {
    ...
    List<StatsDto> findByDate(LocalDateTime start, LocalDateTime end);

    @Query("SELECT NEW ru.practicum.explorewithme.stats.dto.StatsDto(h.app, h.uri, COUNT (DISTINCT h.ip)) FROM Hit AS h " +
            "WHERE h.timestamp BETWEEN :start AND :end " +
            "GROUP BY h.app, h.uri " +
            "ORDER BY COUNT (h.ip) DESC")
    List<StatsDto> findByDateAndUniqueIp(LocalDateTime start, LocalDateTime end);

    @Query("SELECT NEW ru.practicum.explorewithme.stats.dto.StatsDto(h.app, h.uri, COUNT (h.ip)) FROM Hit AS h " +
            "WHERE h.timestamp BETWEEN :start AND :end AND h.uri IN :uris " +
            "GROUP BY h.app, h.uri " +
            "ORDER BY COUNT (h.ip) DESC")
    List<StatsDto> findByDateAndUris(LocalDateTime start, LocalDateTime end, @Param("uris")List<String> uris);

    @Query("SELECT NEW ru.practicum.explorewithme.stats.dto.StatsDto(h.app, h.uri, COUNT (DISTINCT h.ip)) FROM Hit AS h " +
            "WHERE h.timestamp BETWEEN :start AND :end AND h.uri IN :uris " +
            "GROUP BY h.app, h.uri " +
            "ORDER BY COUNT (h.ip) DESC")
    List<StatsDto> findByDateAndUrisWithUniqueIp(LocalDateTime start, LocalDateTime end, @Param("uris") List<String> uris);
}